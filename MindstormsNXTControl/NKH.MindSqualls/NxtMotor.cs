...
namespace NKH.MindSqualls
{
    ...
    public class NxtMotor: NxtPollable
    {
        ...
        public void Run(sbyte power, UInt32 tachoLimit)
        {
            if (reversed) power *= -1;
        ...
        public override void Poll()
        {
            if (Brick.IsConnected)
            {
                ...
                if (oldTachoCount.HasValue && newTachoCount.HasValue)
                {
                    if (OnBelowTachoCount != null &&
                        oldTachoCount > TriggerTachoCount && TriggerTachoCount >= newTachoCount)
                        OnBelowTachoCount(this);
                    else if (OnAboveTachoCount != null &&
                        oldTachoCount <= TriggerTachoCount && TriggerTachoCount < newTachoCount)
                        OnAboveTachoCount(this);
                }
            }
        }
